...
class Graphic:
    ...
    def __create_obstacles(self, ax: plt.Axes) -> list[plt.Rectangle]:
        bottom_wall = self.__create_wall(
            ax,
            (0, -CORRIDOR_WALL_THICKNESS),
            solver.CORRIDOR_LENGTH,
            CORRIDOR_WALL_THICKNESS,
        )
        top_wall = self.__create_wall(
            ax,
            (0, solver.CORRIDOR_WIDTH),
            solver.CORRIDOR_LENGTH,
            CORRIDOR_WALL_THICKNESS,
        )
    ...
    def __create_traces(self, ax: plt.Axes) -> collections.PathCollection:
        ...
        self._trace_segs = np.empty(
            (GRAPHIC_TRACES_MAXLEN, self._n_walkers, 2, 2)
        )  # one segment = 2 points.
        ...
        self._trace_vels = np.empty(
            (GRAPHIC_TRACES_MAXLEN, self._n_walkers)
        )  # norm of the vel.
